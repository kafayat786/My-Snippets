...
if (cluster.isMaster) {
    ...
    server.listen(port, () => {
        if (cluster.worker.id == 1) {//Print Only 1 Time
            console.log(`Listening on port http://${server.address().address}:${server.address().port} At ${new Date().toString()} On ${config.server.nodeEnv} Server as Worker ${cluster.worker.id} running @ process ${cluster.worker.process.pid}`);
        }
    });
}
